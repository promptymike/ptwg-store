...
import { ArrowUpRight, Check, CheckCircle2, Cloud, Smartphone, Sparkles } from "lucide-react";
...
export function PlannerVisual({
  ...
  // Real screenshot of the planner template (generated by
  // scripts/screenshot-planner-thumbs.mjs) — shows the actual product
  // instead of an abstract mockup. The mock below stays as fallback for
  // planners without a capture.
  if (planner.thumbnail) {
    return (
      <div className={`relative isolate overflow-hidden bg-gradient-to-br ${planner.accent} ${compact ? "aspect-[16/10]" : "aspect-[4/3]"}`}>
        ...
        <div className="absolute inset-x-[6%] bottom-0 top-[9%] overflow-hidden rounded-t-[1.1rem] border border-white/25 bg-white shadow-2xl">
          <div className="flex items-center gap-1.5 border-b border-stone-200 bg-stone-50 px-3 py-1.5">
            <span className="size-2 rounded-full bg-rose-400" />
            <span className="size-2 rounded-full bg-amber-300" />
            <span className="size-2 rounded-full bg-emerald-400" />
            <span className="ml-2 truncate text-[9px] font-bold uppercase tracking-[0.16em] text-stone-500 sm:text-[10px]">{planner.shortName}</span>
          </div>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img
            src={planner.thumbnail}
            alt={`Podgląd planera ${planner.name}`}
            loading="lazy"
...
export function PlannerCard({
  ...
  return (
    <article className="group overflow-hidden rounded-[1.6rem] border border-border/70 bg-card/65 shadow-[0_24px_70px_-45px_rgba(0,0,0,.45)] transition duration-500 hover:-translate-y-1 hover:border-foreground/20">
      <Link href={isOwned ? `/narzedzia/${planner.slug}` : `/planery/${planner.slug}`} className="block overflow-hidden">
      ...
      <div className="space-y-5 p-5 sm:p-6">
        ...
        {isOwned ? (
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <Button render={<Link href={`/narzedzia/${planner.slug}`} />}>
